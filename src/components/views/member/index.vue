	...
	export default {
		data() {
			return {
				...
				tableData: {},
				rowsList:member.rowsList,
				queryObj: member.queryForm,
		...
		methods: {
			...
			enable(row) {	//停用/启用会员
				let that = this;
				console.log(row)
				that.$confirm('此操作将停用/启用会员, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					var state = row.status==1?"-1":"1";
					var url = "/member/"+row.code+"/"+state;
			...
			querys() {
				...
				that.isLoading = true;
				var data = this.query
				var url = "/member/pageIndex/" + data.pageIndex + "/pageNum/" + data.pageNum
			...
			deletes(row) { //刪除事件
				let that = this;
				console.log(row)
				that.$confirm('此操作将永久删除选中用户, 是否继续?', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					var url = "/member/"+row.code;
			...
			openModal(val){	//打开积分模态框
				...
				that.editForm.memberCode = val.code
			},
			adds(){
				let that = this;
				var data = that.editForm;
				var url = "/member/"+data.memberCode+"/"+data.oppType+"/"+data.points
				that.$api.put(url, {}).then((res) => {
					console.log(res)
					if (res.ret) {
						that.$message({
